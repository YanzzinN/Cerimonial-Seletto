...
export default function Titulo() {
  ...
  const message = encodeURIComponent("Olá! Vi o site do Cerimonial Seletto e gostaria de agendar um evento.");
  const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${message}`;

  return (
    <section className="relative h-[100dvh] w-full flex flex-col items-center justify-center overflow-hidden">
      <div className="absolute inset-0 z-0">
        <Image src="/seletto1.webp" alt="Background" fill priority className="object-cover" />
      </div>
      <div className="absolute inset-0 bg-black/60 z-10" />

      <div className="relative z-30 flex flex-col items-center text-center px-6 max-w-4xl">
        {/* Ajuste: text-5xl no mobile e leading-tight para não encavalar */}
        ...
        {/* Ajuste: text-base no mobile para melhor leitura */}
        ...
        {/* Ajuste: flex-col no mobile para botões não apertarem */}
        <div className="mt-10 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto">
            <button className="w-full flex items-center justify-center gap-2 px-8 py-4 rounded-full border border-white/40 bg-white/10 text-white backdrop-blur-md hover:bg-white hover:text-black transition-all group">
              <MessageCircle size={20} className="text-green-400 group-hover:text-black transition-colors" />
              <span className="font-medium text-sm md:text-base">Agende seu evento</span>
            </button>
          </a>
          <button 
